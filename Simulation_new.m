...
if Sim1{1,i+1}(2,j) > 9000000
 ...
 Sim1{1,i+1}(24,j)= LOS(ROW_Detail{1,i},Conflict_1{1,i},airplane,j,Distance_XY{1,i},conflict_separation_1,conflict_separation_2,Sim1{1,i+1}(4,j)); % Lost of Separation
   ...
   Sim1{1,i+1}(30,j) = clearance(ROW_Detail{1,i},Conflict_1{1,i},airplane,j,Distance_XY{1,i},conflict_separation_1,conflict_separation_2,Sim1{1,i+1}(4,j),Holding_status{1,i}(1,j)); %Status Conflict
 
%%% Approach Controller %%% 
if Sim1{1,i}(29,j)== (1 || 2) && Sim1{1,i+1}(29,j)== 3
   APPTW{1,i}(1,j)= 1; % Enter sector Terminal West Check
else
    APPTW{1,i}(1,j)= 0;
end

if Sim1{1,i}(29,j)== 3 && Sim1{1,i+1}(29,j)== 4
   APPTW{1,i}(2,j)= 1; % Exit sector Terminal West Check
else
    APPTW{1,i}(2,j)= 0;
end
 
if Sim1{1,i}(29,j)== 4 && Sim1{1,i+1}(29,j)== 3
   APPTW{1,i}(3,j)= 1; % Radar Identification
else
    APPTW{1,i}(3,j)= 0;
end

if Sim1{1,i}(9,j)== 0 && Sim1{1,i+1}(29,j)== 2
   APPTW{1,i}(4,j)= 1; % Radar vector
else
    APPTW{1,i}(4,j)= 0;
end

if Sim1{1,i}(9,j)== 0 && Sim1{1,i}(10,j)~= 2
   APPTW{1,i}(5,j)= 1; % Control conflict
else
    APPTW{1,i}(5,j)= 0;
end

if i==1
    APPTW{1,i}(6,j)= 0; % Slot time for Communication time of coordination in sector 
    APPTW{1,i}(7,j)= 0; % Slot time for Communication time of coordination out sector
     APPTW{1,i}(8,j)= 0; % Slot time for Communication time of Radar Identification 
    APPTW{1,i}(9,j)= 0; % Slot time for Communication time for Radar vector
    APPTW{1,i}(10,j)= 0; % Slot time for Communication time for Control conflict
else
end
    

 if APPTW{1,i}(1,j)== 1 && slot(1,i)==0
    tcin = 16;% Communication time for coordination in sector
    APPTW{1,i+1}(6,j)= 1;
 elseif APPTW{1,i}(6,j)== 1 && slot(1,i)==1 && tcin>0
     tcin = tcin-1;
     APPTW{1,i+1}(6,j)= 1;
 elseif APPTW{1,i}(6,j)== 1 && slot(1,i)==1 && tcin==0
     tcin = 0;
 ...
 if APPTW{1,i}(3,j)== 1 && slot(1,i)==0
    tcout = 14; % Communication time for coordination out sector
    APPTW{1,i+1}(7,j)= 1;
 elseif APPTW{1,i}(7,j)== 1 && slot(1,i)==1 && tcout>0
     tcout = tcout-1;
     APPTW{1,i+1}(7,j)= 1;
 elseif APPTW{1,i}(7,j)== 1 && slot(1,i)==1 && tcout==0
     tcout = 0;
  ...
  if APPTW{1,i}(4,j)== 1 && slot(1,i)==0
    tcout = 12; % Communication time for radar identification 
    APPTW{1,i+1}(8,j)= 1;
 elseif APPTW{1,i}(8,j)== 1 && slot(1,i)==1 && tcout>0
     tcout = tcout-1;
     APPTW{1,i+1}(8,j)= 1;
 elseif APPTW{1,i}(8,j)== 1 && slot(1,i)==1 && tcout==0
     tcout = 0;
 ...
 if APPTW{1,i}(5,j)== 1 && slot(1,i)==0
    tcout = 16; % Communication time for radar vectoring 
    APPTW{1,i+1}(9,j)= 1;
 elseif APPTW{1,i}(9,j)== 1 && slot(1,i)==1 && tcout>0
     tcout = tcout-1;
     APPTW{1,i+1}(9,j)= 1;
 elseif APPTW{1,i}(9,j)== 1 && slot(1,i)==1 && tcout==0
     tcout = 0;
  ...
  if APPTW{1,i}(2,j)== 1 && slot(1,i)==0
    tcout = 15; % Communication time for control conflict
    APPTW{1,i+1}(10,j)= 1;
 elseif APPTW{1,i}(10,j)== 1 && slot(1,i)==1 && tcout>0
     tcout = tcout-1;
     APPTW{1,i+1}(10,j)= 1;
 elseif APPTW{1,i}(10,j)== 1 && slot(1,i)==1 && tcout==0
     tcout = 0;
 ...
 if sum(APPTW{1,i+1}(6,:))== 1 || sum(APPTW{1,i+1}(7,:))== 1 || sum(APPTW{1,i+1}(8,:))== 1 || sum(APPTW{1,i+1}(9,:))== 1 || sum(APPTW{1,i+1}(10,:))== 1
     slot(1,i+1)=1;
 else   
     slot(1,i+1)=0; 
 end
